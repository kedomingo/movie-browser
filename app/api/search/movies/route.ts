...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (query) {
      data = await tmdbClient.searchMovies({
        query,
      ...
      if (year) {
        ...
        if (yearRange.gte) {
          discoverParams["release_date.gte"] = yearRange.gte;
        }
        if (yearRange.lte) {
          discoverParams["release_date.lte"] = yearRange.lte;
        }
      }
      ...
      data = await tmdbClient.discoverMovies(discoverParams);
    }
